...
class AgentUi(tk.Canvas):
    ...
    def render(self, event: tk.Event = None) -> None:
        ...
        self.create_window(width//2 - 10, 50, anchor="ne", width=width//2-10, window=property_frame)
        ...
        self.create_window(width//2 + 10, 50, anchor="nw", width=width//2-10, window=transaction_frame)
        ...
        if transactions is not None:
            for i in range (len(transactions)):
                transaction_tree.insert(parent='', index='end', iid=i, values=localities[i])
        transaction_tree.pack(fill = tk.X)
